...
# Take url as an argument at execution and store it in web_url
web_url = str(sys.argv[1])

# Install Chrome Driver and instantiate driver instance
...
# Create Data Frame for Match Stat data
col_names = ['Date', 'homeTeam', 'awayTeam', 'homeGoals', 'awayGoals', 'result']
match_stats = pd.DataFrame(columns = col_names)

# Scrape web elements for pertinent data for Match Stats Data Frame
...
for row in table_body.find_elements(By.TAG_NAME,'tr'):
	...
	if (row.get_attribute("class") == "spacer partial_table result_all" or row.get_attribute("class") == "thead"):
		...
		score = row.find_elements(By.TAG_NAME, "td")[5].text.split("–")
		...
		if (home_goals > away_goals):
			...
			result = "D"
		new_match = pd.DataFrame({"Date": [row.find_elements(By.TAG_NAME, "td")[1].text], "homeTeam" : [row.find_elements(By.TAG_NAME, "td")[3].text], "awayTeam": [row.find_elements(By.TAG_NAME, "td")[7].text], "homeGoals": [home_goals], "awayGoals": [away_goals], "result" : [result]})
		match_stats = pd.concat([match_stats, new_match])
		
print(match_stats.head())

# Create file name and save data as csv
